...
def is_particle_preserving(circuit, n):
    ...
    preserving=True
    for state in states:
        hammingwgt_int = hammingWeight(state)
        newState = circuit(state)

        for index in range(len(newState)):

            amplitude = newState[index]
            if amplitude != 0:
                #print('Comparing {} to {}'.format(state, states[index]))
                #print('Hamming wgts {} to {}'.format(hammingwgt_int, hammingWeight(states[index])))
                preserving = (hammingwgt_int == hammingWeight(states[index]))
