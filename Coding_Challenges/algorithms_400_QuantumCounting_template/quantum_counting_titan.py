...
def circuit(indices):
    ...
    target_wires = range(4) #could be trouble

    estimation_wires = range(4, 8) #could be trouble
...
def number_of_solutions(indices):
    ...
    maxwire = np.argmax(probs)


    theta = maxwire*np.pi/8.
...
def relative_error(indices):
    ...
    # QHACK #
    nsolutions_est = number_of_solutions(indices)
    rel_err = (nsolutions_est - len(indices))/len(indices) *100.
