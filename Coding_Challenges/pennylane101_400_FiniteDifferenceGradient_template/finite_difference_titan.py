...
def my_finite_diff_grad(params):
    ...
    for i in range(len(params)):
        # QHACK # 
        delta = 0.00001
        nuparams_plus = np.copy(params)
        nuparams_minus = np.copy(params)
        nuparams_plus[i] = params[i] + delta/2.
        nuparams_minus[i] = params[i] - delta / 2.
        gradients[i] = (cost(nuparams_plus) - cost(nuparams_minus)) /delta
